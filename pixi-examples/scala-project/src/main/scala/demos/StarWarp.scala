...
object StarWarp extends PIXIExample {
  ...
  val pixiUrl: String = "https://pixijs.io/examples/#/demos/star-warp-sprites.js"

  def newApplication(): Application = {
    val app = new Application(new ApplicationOptions {
      override val backgroundColor: UndefOr[Int] = 0x0
    })

    //Get the texture for rope.
    val starTexture = Texture.fromImage("required/assets/star.png")
    ...
    def randomizeStar(star: Star, initial: Boolean): Unit = {
      star.z = if (initial) Math.random()*2000 else cameraZ + Math.random()*1000+2000
    ...
    setInterval(5000) {
      warpSpeed = if (warpSpeed > 0) 0.0 else 1.0
    }
    ...
    def ticker(delta: Double): Unit = {
      ...
      for (star <- stars) {
        ...
        val distanceCenter = Math.sqrt( dxCenter*dxCenter + dyCenter + dyCenter)
    ...
    app.ticker.add((delta: Double) => ticker(delta))
